...
![user flow diagram before](blg_s3_cf_flow1.png)
...
![user flow diagram after](blg_s3_cf_flow2.png)
...
Given my requirements it still might be better to use an SQS queue in the middle to ensure event delivery and provide some ordering and possibly a little more error handling for failed processing; but as this is just a case of picking up that a S3 object key has changed then firing its path over to an external API it seemed unnecessary. This was also a pretty fun way to try out this new-ish functionality.
